...
struct pcb_t *proc_init(void) {
	static struct pcb_t pcb_table[MAXPROC]; //Array statico contenente MAXPROC pcb_t 
	int i;
	...
	for(i=1; i<MAXPROC; i++){ //Aggiunge tutti i pcb_t (a parte il root) nella lista libera
		struct pcb_t* newPcb = &pcb_table[i];
        	list_add(&(newPcb->p_siblings), &(pcbFree)); //p_siblings usata per gestire la lista libera
	}
	struct pcb_t* root=&(pcb_table[0]); //root non è stato messo nella pcbFree
...
struct pcb_t *proc_alloc(struct pcb_t *p_parent){
		...
		struct pcb_t* allocpcb = container_of(pcbFree.next, struct pcb_t, p_siblings);
...
struct pcb_t *proc_firstchild(struct pcb_t *proc){
	...
	struct pcb_t *firstchild = container_of((proc->p_children).next, struct pcb_t, p_siblings); //primo figlio
	return (firstchild);
}
...
struct tcb_t *proc_firstthread(struct pcb_t *proc){
	...
	struct tcb_t *firstthread = container_of((proc->p_threads).next, struct tcb_t, t_next); //primo thread
	return (firstthread);
}
...
void thread_init(void) {
	static struct tcb_t tcb_table[MAXTHREAD]; //Array statico contenente MAXTHREAD tcb_t 
	int i;
	INIT_LIST_HEAD(&(tcbFree)); //Inizializza la testa della lista libera tcbFree
	for(i=0; i<MAXTHREAD; i++){ //Aggiunge tutti i tcb_t della tcb_table nella lista libera
		struct tcb_t* newTcb = &tcb_table[i];
...
struct tcb_t *thread_alloc(struct pcb_t *process) {
	...
	struct tcb_t* allocTcb = container_of(tcbFree.next, struct tcb_t, t_next);
	//Stacco l'elemento dalla lista libera
...
int thread_free(struct tcb_t *oldthread){
	...
	list_add(&(oldthread->t_next), &(tcbFree));
...
struct tcb_t *thread_qhead(struct list_head *queue){
	...
	return (container_of(queue->prev, struct tcb_t, t_sched));
	//ERRORE FIFO, non ci va next perchè inseriamo in testa
}
...
struct tcb_t *thread_dequeue(struct list_head *queue){
	if (list_empty(queue)) return NULL;
	struct list_head *temp_t = queue->prev;
	list_del(queue->prev);
	return (container_of(temp_t, struct tcb_t, t_sched));
}
...
/* the return value is the address of the root process */
void msgq_init(void){
	static struct msg_t msg_table[MAXMSG]; //Array statico contenente MAXMSG msg_t 
	int i;
	...
	for(i=0; i<MAXMSG; i++){ //Aggiunge tutti i msg_t della msg_table nella lista libera
		struct msg_t* newMsg = &msg_table[i];
...
int msgq_add(struct tcb_t *sender, struct tcb_t *destination, uintptr_t value){
	...
	struct msg_t* allocMsg = container_of(msgFree.next, struct msg_t, m_next);
	//Stacco l'elemento dalla lista libera
	...
	list_add_tail(&(allocMsg->m_next), &(destination->t_msgq));
	return 0;
}
...
int msgq_get(struct tcb_t **sender, struct tcb_t *destination, uintptr_t *value){
	if (sender==NULL){ //CASO 1: Sender NULL - No filtro mittente
		...
		struct msg_t *temp_m = container_of((destination->t_msgq).next, struct msg_t, m_next);
		//Elimino temp_m dalla lista t_msgq del destinatario		
		list_del(&(temp_m->m_next));
